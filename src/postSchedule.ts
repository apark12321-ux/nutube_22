...
const FILL_END_UTC = Date.UTC(2026, 6, 11, 23, 59, 59);
...
export const applyPostDateSchedule = (posts: GuidePost[]): GuidePost[] => {
  ...
  const SUBTITLE_OPTIMIZATION_MAP: Record<string, string> = {
    ...
    '독자의 자발적인 소통 광장을 개조하고 호감형 알고리즘 점수를 유발하는 3선 답글 작법': '시청자 참여율을 높이고 커뮤니티 활성화를 유도하는 실전 답글 가이드',
    '포털 크롤러를 내 사이트로 무조건 자석처럼 끌어당기는 고유 캐노니컬 태그 설계와 네이버/구글 동시 색인 기각 해소법': '발견됨 또는 크롤링됨 현재 색인 생성되지 않음 원인과 해결책',
    '가치 없는 콘텐츠와 중복 계정 탈락 사슬을 정면 돌파하고 광고 수익 채널 활성화하기': '가치 없는 콘텐츠 해결 및 전문성(E-E-A-T) 신호 강화',
    ...
    '3분 확장 이후 바뀐 핵심 신호와 첫 3초 전략': '동영상 길이 연장에 따른 알고리즘 신호 대응 방법',
