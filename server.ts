...
최종 응답은 반드시 사전에 정의된 JSON 스키마에 부합하는 아름다운 JSON 포맷이어야 한다.`;
      ...
      return res.status(500).json({ 
        error: 'Gemini 모델 생성 중 오류가 발생했습니다.', 
        details: err.message 
      });
    }
  }
...
#유튜브성장 #유튜브알고리즘 #시니어쇼츠 #유튜브수익화 #NuTube`,
    tags: ["유튜브성장", "유튜브알고리즘", "시니어사연쇼츠", "유튜브쇼핑제휴", "썸네일기획", "유튜브대본", "AI영상제작", "NuTube"],
    storyboard: [
      {
        scene: "Scene 1: 첫 3초의 강박적 후킹",
        visual: "카메라 줌인. 심각한 표정의 화자 혹은 고대비의 낡은 가방 실물 단독 샷. 하단 자막에 흰색 굵은 폰트 배치 후 흔들리는 모션.",
        audio: "BGM: 긴장감 넘치는 로우 파이 베이스음 단발성 타격. 내레이션: \"솔직히 말씀드릴게요. 여러분이 매일 올리던 이 영상, 어쩌면 전부 멈추셔야 합니다.\"",
        timing: "00:00 - 00:03"
      },
      {
        scene: "Scene 2: 문제 상황의 심리학적 제기",
        visual: "조회수가 뚝 떨어져 정체된 스튜디오 분석 차트가 화면에 크게 흐려지며 지나감. 구체적이고 붉은색 패닉 노선 화살표 표시.",
        audio: "BGM: 미디엄 템포의 차분하며 집중이 잘 되는 신디사이저 저음. 내레이션: \"열심히 하루 종일 공들였는데 조회수 100회 언더에 머물러 낙심하셨나요? 진짜 범인은 썸네일이 아니라 바로 이것입니다.\"",
        timing: "00:03 - 00:15"
      },
      {
        scene: "Scene 3: 마이크로 니치의 과학적 원리 제공",
        visual: "다양한 주제(요리, 요가, 사연 등)가 알록달록한 클러스터 동그라미로 깔끔하게 묶이는 직관적인 그래픽 효과. 화자의 활기찬 손짓 모션.",
        audio: "BGM: 희망차고 깔끔하며 리드미컬한 업비트 음원. 내레이션: \"2026년 알고리즘은 큰 카테고리가 아니라 더욱 세밀한 세션 클릭 이력을 기반으로 좁은 매칭률을 선사합니다. 정직하게 좁힐 수록 노출 확률은 열 배 이상 상승합니다.\"",
        timing: "00:15 - 00:45"
      },
      {
    ...
    thumbnails: [
      {
        graphic: "배경은 감성적이고 약간 어두운 방안 조명, 한쪽에 낡은 보물상자가 놓여 있음. 사방에 밝은 조명 후광 효과 탑재.",
    ...
    shortsScript: {
      hook: "유튜브 한창 채널 올리시는 분들, 딱 30초만 들어보세요. 이거 모르면 6개월 동안 올린 영상 아예 노출 제로 됩니다.",
      body: "알고리즘이 올해부터 전적으로 가중치를 높인 건 단 하나, 재시청과 댓글 참여도입니다. 영상을 아무리 길고 화려하게 다듬어봤자, 시청자가 첫 3초 만에 넘기거나 의견 하나 남기지 않고 나가버리면 알고리즘은 쓰레기 콘텐츠로 직행 분류합니다. 오늘부터 무조건 영상 마지막 컷에 '시청자가 자기 의견을 한 줄 남길 수밖에 없는 구체적인 질문'을 자막으로 박으세요. 이것 하나로 댓글이 세 배 늘고 노출이 다섯 배 커집니다.",
...
app.post('/api/assistant/chat', async (req, res) => {
  ...
  const systemInstructions: Record<string, string> = {
    ...
    senior: "너는 '감성 사연 디렉터'이다. 다정하고 부드러우며 마음을 돌보는 따뜻한 말투를 쓴다. 시니어 시청자들의 마음을 따뜻하게 가라앉힐 수 있는 진심어린 스토리텔링과 정직함에 관한 팁을 건넨다.",
  ...
  if (ai) {
    try {
      ...
      return res.status(500).json({ 
        error: 'Gemini 채팅 응답 중 오류가 발생했습니다.', 
        details: err.message 
      });
    }
  }
